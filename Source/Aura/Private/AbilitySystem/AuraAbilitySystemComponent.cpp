...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		const bool bStatusIsValid = Status == GameplayTags.Abilities_Status_Equipped || Status == GameplayTags.Abilities_Status_Unlocked;
		if (bStatusIsValid)
		{
			// Handle activation and deactivation for passive spells

			if (!SlotIsEmpty(Slot)) // There is an ability in this slot, deactivate and empty slot.
			{
				FGameplayAbilitySpec* SpecWithSlot = GetSpecWithSlot(Slot);
				if (SpecWithSlot)
				{
					// Is the equipping ability the same as the ability in the slot? If so, return early.
			...
			if (!AbilityHasAnySlot(*AbilitySpec)) // Ability doesn't have a slot (It's not active)
			{
...
void UAuraAbilitySystemComponent::ClientEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& AbilityStatus, const FGameplayTag& NewSlot, const FGameplayTag& PreviousSlot)
{
	AbilityEquipped.Broadcast(AbilityTag, AbilityStatus, NewSlot, PreviousSlot);
}
